...
def log_likelihood(X,t,y,sig):
    ...
    nloglikelihood = (gaussian_vals-y)**2/(2*sig_fit**2) - np.log(1/(2.506*sig_fit))
    ...
    for fn in fn_array:
        if 'fluxcal' in fn:
            if '.npz' in fn:
                profile = np.load(fn)
                data = profile['data']
                ds_data = 0.25*(data[:,0::4] + data[:,1::4]+data[:,2::4]+data[:,3::4])
                print(fn)
                ...
                print(peak)
                max_l = minimize(log_likelihood,[peak,peak_loc,0.01,1],args=(tx,summed,sig),method='Nelder-Mead')
                x = max_l.x
                ...
                def update(val):
                    ...
                    max_l = minimize(log_likelihood,[peak,peak_loc,sigma,a],args=(tx,summed,sig),method='Nelder-Mead')
                    global x_new
                    x_new = max_l.x
                    print('new fit: ',x_new)
                ...
                if not isinstance(x_new,bool):
                    ...
                    pass
                print(x)
                bursts_characteristics.append({'fn':fn,'data':summed,'dt':dt,'char':x,'x_order':['peak','peak_loc','width','offset']})
