...
for fn in sys.argv[1:]:
    on_phase_edges = [0.65,0.9]
    off_phase_edges = [0.1,0.35]
    profiles_np,mask_np = process_archive(fn)
    on_temp,off_temp = calculate_on_off_intensities(profiles_np,mask_np,on_phase_edges,off_phase_edges)
...
means_fit, means_err, stds_fit, stds_err, weights_fit, weights_err, samples, lnprobs=NP.fit_mcmc(nwalkers=40,niter=500,ninit=50,nthreads=25)
import corner
