...
def extract_plot_data(
    data, masked_chans, dm, downsamp, nsamps_start_zoom, nsamps_end_zoom
):
    ...
    waterfall_dat = waterfall_dat[
        :,
        int(nsamps_start_zoom / (downsamp * second_downsamp)) : int(
            nsamps_end_zoom / (downsamp * second_downsamp)
        ),
    ]
...
def grab_spectra_manual(
    ...
    waterfall_dat, dat_ts = extract_plot_data(
        data, masked_chans, dm, downsamp, nsamps_start_zoom, nsamps_end_zoom
    )
    if manual:
        ...
        if (amp != -1) & (
            (loc < (0.46 * t_dur)) | (loc > (t_dur * 0.54)) | (sigma_width > 2e-2)
        ):
...
def find_polynomial_fit(x_std, ts_std, order=None):
    ...
    if order is None:
        for i in range(1, 10):
            ...
            rchi2 = np.sum(
                (ts_std - poly(x_std)) ** 2 / (np.std(ts_std[1:500]) ** 2)
            ) / (len(ts_std) - i)
...
def autofit_pulse(
    ...
    xind = x
    print(
        f"initial width guess {fit_width_guess}, amp {mamplitude}, max_time {max_time}"
    )
    ...
    if plot:
        ...
        axs[0, 1].set_title("baseline removed")
    ...
    print(
        f"Fitted loc:{loc} amp:{Amplitude} std:{std} width sigma:{sigma_width} fluence:{fluence}"
    )
...
def fit_SNR_manual(ts, tsamp, width, nsamps, ds_data, downsamp):
    ...
    w = Slider(w_ax, "width", 0.0, 0.05, valinit=fitx[2], valstep=1e-3)
    fit_order = Slider(
        fit_order_ax, "fit order", 0, 10, valinit=len(coeffs) - 1, valstep=1
    )
...
class inject_obj:
    def __init__(self, snr=1, width=1, toas=1, dm=1, downsamp=8, filfile="", mask=""):
        self.snr = snr
        self.width = width
        self.toas = toas
        self.dm = dm
    ...
    def calculate_fluence_single(self, mask=True, period=2, manual=True, plot_name=""):
        ...
        if self.det_amp == -1:
            ...
            self.processed = True
        print(f"fitted_snr {det_snr} std {std} amp {amp}")
    ...
    def calculate_fluence(self, plot_folder="temp"):
        ...
        for t, dm, snr, width in zip(self.toas, self.dm, self.snr, self.width):
            ...
            print(f"inj_snr {snr} fitted_snr {det_snr} std {std} amp {amp}")
            # print(f"Calculated fluence:{fluence} A:{amp} S:{std} Nominal FLUENCE:{self.fluence}")
...
class inject_stats:
    ...
    def match_inj(
        ...
        for f, m in zip(self.filfiles, self.mask_fn):
            ...
            if len(cur_snr) == 0:
                print(
                    f"WARNING NO MATCHING SNR, THIS SHOULD NOT HAPPEN NORMALLY. IF YOU ARE NOT EXPECTING THIS WARNING THEN CHECK THE FILES CREATED {f}"
                )
                continue
    ...
    def amplitude_statistics(self, title="f"):
        ...
        unique_snrs, unique_widths, det_matrix_width = create_matrix(
            inj_snr, inj_width, det_width, norm=1
        )
        unique_snrs, unique_widths, det_matrix_snr = create_matrix(
            inj_snr, inj_width, det_snr, norm=0
        )
        unique_snrs, unique_widths, det_matrix_width_std = create_matrix(
            inj_snr, inj_width, det_width_std, norm=-1
        )
        unique_snrs, unique_widths, det_matrix_snr_std = create_matrix(
            inj_snr, inj_width, det_snr_std, norm=-1
        )
        # convert to ms
        ...
        mesh = axes[1, 0].pcolormesh(
            unique_widths, unique_snrs, det_matrix_width_std * 1000
        )
        mesh.set_clim(0, 10)
        ...
        if hasattr(self, "base_fn"):
            ...
            pass
            # plt.show()
        unique_fluences, unique_width_fs, det_matrix_width_f = create_matrix(
            inj_fluence, inj_width, det_width, norm=1
        )
        unique_fluences, unique_width_fs, det_matrix_fluence = create_matrix(
            inj_fluence, inj_width, det_fluence, norm=0
        )
        unique_fluences, unique_width_fs, det_matrix_width_f_std = create_matrix(
            inj_fluence, inj_width, det_width_std, norm=-1
        )
        unique_fluences, unique_width_fs, det_matrix_fluence_std = create_matrix(
            inj_fluence, inj_width, det_fluence_std, norm=-1
        )
        # convert to ms
        ...
        mesh = axes[0, 0].pcolormesh(
            unique_width_fs, unique_fluences, det_matrix_width_f
        )
        mesh.set_clim(0.8, 1.2)
        cbar = plt.colorbar(mesh, ax=axes[0, 0])
        ...
        mesh = axes[0, 1].pcolormesh(
            unique_width_fs, unique_fluences, det_matrix_fluence
        )
        mesh.set_clim(0.8, 1.2)
        cbar = plt.colorbar(mesh, ax=axes[0, 1])
        ...
        mesh = axes[1, 0].pcolormesh(
            unique_width_fs, unique_fluences, det_matrix_width_f_std * 1000
        )
        mesh.set_clim(0, 10)
        cbar = plt.colorbar(mesh, ax=axes[1, 0])
        ...
        mesh = axes[1, 1].pcolormesh(
            unique_width_fs, unique_fluences, det_matrix_fluence_std
        )
        # mesh.set_clim(0,1)
        cbar = plt.colorbar(mesh, ax=axes[1, 1])
        ...
        self.detect_error_fluence = np.sqrt(
            np.nanmean(det_matrix_fluence_std[fmask] ** 2)
        )

        self.detect_error_snr_low_width = np.sqrt(
            np.nanmean(det_matrix_snr_std[low_width_mask] ** 2)
        )
        self.detect_error_width_low_width = np.sqrt(
            np.nanmean(det_matrix_width_std[low_width_mask] ** 2)
        )
        print(
            f"low width SNR error: {self.detect_error_snr_low_width}, low width width error: {self.detect_error_width_low_width}"
        )
        print(
            f"SNR error: {self.detect_error_snr}, width error: {self.detect_error_width}, fluence error: {self.detect_error_fluence}"
        )
    ...
    def compare(self, fn, plot=True, title="detection_curve"):
        ...
        # get lists to plot
        unique_snrs, unique_widths, det_frac_matrix_snr = create_matrix(
            inj_snr, inj_width, det_frac, norm=2
        )
        unique_fluence, unique_width_fs, det_matrix_fluence = create_matrix(
            inj_fluence, inj_width, det_frac, norm=2
        )
        ...
        detected_amplitudes_fluence = list(
            s.det_fluence[s.detected] for s in detected_sample
        )
        ...
        # flatten the lists
        detected_amplitudes_snr = [
            item for sublist in detected_amplitudes_snr for item in sublist
        ]
        detected_amplitudes_fluence = [
            item for sublist in detected_amplitudes_fluence for item in sublist
        ]
        detected_widths = [item for sublist in detected_widths for item in sublist]
        all_det_amplitudes_snr = [
            item for sublist in all_det_amplitudes_snr for item in sublist
        ]
        all_det_amplitudes_fluence = [
            item for sublist in all_det_amplitudes_fluence for item in sublist
        ]
        ...
        self.bin_detections_2d(
            self.all_det_amplitudes_snr,
            self.detected_amplitudes_snr,
            self.all_det_widths,
        ...
        self.bin_detections_2d(
            self.all_det_amplitudes_fluence,
            self.detected_amplitudes_fluence,
            self.all_det_widths,
            self.detected_widths,
            num_bins=num_bins,
            plot=False,
        ...
        fig, axes = plt.subplots(2, 2, figsize=(10, 10))
        mesh = axes[0, 0].pcolormesh(
            unique_widths * 1000, unique_snrs, det_frac_matrix_snr
        )
        mesh.set_clim(0, 1)
        cbar = plt.colorbar(mesh, ax=axes[0, 0])
        ...
        mesh = axes[1, 0].pcolormesh(
            unique_width_fs * 1000, unique_fluence, det_matrix_fluence
        )
        mesh.set_clim(0, 1)
        cbar = plt.colorbar(mesh, ax=axes[1, 0])
        cbar.set_label("Detection Fraction")
        ...
        mesh = axes[1, 1].pcolormesh(
            self.detected_bin_midpoints_fluence[1] * 1000,
            self.detected_bin_midpoints_fluence[0],
            self.detected_det_frac_fluence,
        )
        cbar = plt.colorbar(mesh, ax=axes[1, 1])
        mesh.set_clim(0, 1)
        cbar.set_label("Detection Fraction (binned)")
    ...
    def bin_detections(self, all_det_vals, detected_det_vals, num_bins=20, plot=False):
        ...
        if plot:
            ...
            axes[0].scatter(
                bin_midpoints, detected_det_frac, label="P(Detected|amplitude_detected)"
            )
    ...
    def bin_detections_2d(
        ...
        all_mask = (
            (all_det_vals < (max(self.inj_snr) + 0.5))
            & (all_width_vals < (max(self.inj_width) + 1e-3))
            & (all_width_vals > 2e-3)
        )
        det_mask = (
            (detected_det_vals < (max(self.inj_snr) + 0.5))
            & (detected_width_vals < (max(self.inj_width) + 1e-3))
            & (detected_width_vals > 2e-3)
        )
        ...
        if min(all_det_vals) > 0:
            bin_edges_snr = np.logspace(
                np.log10(min(all_det_vals)), np.log10(max(all_det_vals)), num_bins + 1
            )
            bin_edges_width = np.logspace(
                np.log10(min(all_width_vals)),
                np.log10(max(all_width_vals)),
                num_bins + 1,
            )
        else:
            print("log spacing error, found negative numbers, defaulting to linear")
            bin_edges_snr = np.linspace(
                min(all_det_vals), max(all_det_vals), num_bins + 1
            )
            bin_edges_width = np.linspace(
                min(all_width_vals), max(all_width_vals), num_bins + 1
            )
        ...
        all_2d_hist, xedges, yedges = np.histogram2d(
            all_det_vals, all_width_vals, bins=(bin_edges_snr, bin_edges_width)
        )
        detected_2d_hist, xedges, yedges = np.histogram2d(
            detected_det_vals,
            detected_width_vals,
            bins=(bin_edges_snr, bin_edges_width),
        )
