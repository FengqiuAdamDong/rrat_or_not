...
def inject_pulses(
    ...
    for i, p in enumerate(pulse_attrs):
        ...
        # statistics.append(calculate_SNR_wrapper(p,stats_window,tsamp,1,copy.deepcopy(data),masked_chans,plot=True))
...
def calculate_SNR_wrapper(p,stats_window,tsamp,downsamp,data,masked_chans,plot):
    ...
    extra_time_bins = time_to_bin(5-stats_window,tsamp)
    ...
    #get the window
    focused_stats_start = time_to_bin(5-stats_window,tsamp*downsamp)
    focused_stats_end = time_to_bin(5-stats_window+2*stats_window,tsamp*downsamp)
    ...
    plt.plot(stats_mean)
    plt.show()
    amp,std,loc,sigma_width =  autofit_pulse(stats_mean,tsamp*downsamp,0.1,int(stats_window/tsamp/downsamp)
