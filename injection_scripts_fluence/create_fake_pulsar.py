...
def create_lognorm_pulses(
    mu, std, p, f, d, dm, downsamp=3, stats_window=0.9, sigma_snr=0.3
):
    # always start 5 seconds in and end 5 seconds early
    d = d - 10

    pulse_snrs = simulate_pulses(obs_t=d, period=p, f=f, mu=mu, std=std)
...
if __name__ == "__main__":
    ...
    pulse_attributes, stats_window, downsamp = create_lognorm_pulses(
        mu=mu, std=std, p=p, f=f, d=d, dm=DM, sigma_snr=SNR_std
    )
    inject_params = (
        DM,
        FN_ext,
