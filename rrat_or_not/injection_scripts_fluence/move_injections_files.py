...
if __name__ == "__main__":
    ...
    argparser.add_argument(
        ...
        type=str,
        default="injections",
        help="path to store the injections filterbank files (just a symlink)",
    )
    argparser.add_argument("--gap", type=int, help="gap between two filterbank files")
    argparser.add_argument(
        "--symlink", default=True, action="store_false", help="symlink or copy"
    )
    ...
    for i in range(0, len(files), gap):
        ...
        if symlink:
            if not os.path.exists(os.path.join(pwd, os.path.join(path, active_file))):
                os.symlink(
                    os.path.join(pwd, active_file),
                    os.path.join(pwd, os.path.join(path, active_file)),
                )
        else:
            if not os.path.exists(os.path.join(pwd, os.path.join(path, active_file))):
        ...
        rfifind_files = glob.glob(
            os.path.join(os.getcwd(), os.path.join(basename, "*rfifind*"))
        )
        ...
        for rfifind_file in rfifind_files:
            ...
            if not os.path.exists(rfi_path):
                ...
                os.symlink(
                    rfifind_file, new_folder + "/" + os.path.basename(rfifind_file)
                )
