...
def create_lognorm_pulses(
    mu, std, mu_w, std_w, p, f, d, dm, downsamp=3, stats_window=0.9
):
    # always start 5 seconds in and end 5 seconds early
    inj_d = d - 20
    print("simulating pulses")
    pulse_snrs = simulate_pulses(
        obs_t=inj_d,
        period=p,
        f=f,
        mu=mu,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-d", help="Duration of required output file", type=float, default=700
    )
    ...
    parser.add_argument(
        "-ext",
        help="this is the extension of the output file, integer",
        type=int,
        default=2345,
    )
    ...
    d = args.d
    maskfn = args.m
    pulse_attributes, stats_window, downsamp = create_lognorm_pulses(
        mu=mu, std=std, mu_w=mu_w, std_w=std_w, p=p, f=f, d=d, dm=DM
    )
    print(
        f"injecting at dm {DM} with pulse period {p} and pulse fraction {f} into file with duration {d}"
    )
    inject_params = (
        DM,
        FN_ext,
