...
def grab_spectra(gfb,ts,te,mask_fn,p,downsamp,window):
    #load the filterbank file
    g = r.FilReader(gfb)
...
if __name__ == "__main__":
    ####THIS IS FOR MANUAL RECOVERY OF INJECTION SNR##########
    ...
    injections = np.load("sample_injections.npz",allow_pickle=1)
    ds = injections['downsamp'].tolist()
    stats_window = injections['stats_window'].tolist()
    ...
    for inj in injections['grid']:
        t, snr, dm = inj
        mask_arr = get_mask_arr([bfb])
        ts_arr = t-(stats_window)
        te_arr = t+(stats_window)
        print(mask_arr,ts_arr,te_arr)
        grab_spectra(bfb,ts_arr,te_arr,mask_arr[0],inj,ds,stats_window)
