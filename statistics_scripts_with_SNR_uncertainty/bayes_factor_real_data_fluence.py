...
if __name__ == "__main__":
    ...
    mu_arr = np.linspace(-0.25, 0.25, mesh_size)
    std_arr = np.linspace(0.3, 0.6 ,mesh_size + 1)
    N_arr = np.linspace(len(det_snr), obs_t/p, mesh_size + 2)
    # N_arr = np.linspace(200, obs_t/p , mesh_size + 2)
    ...
    k_arr = np.linspace(0.1, 1, exp_mesh_size)
    # k_arr = np.array([9.9,10.1])
