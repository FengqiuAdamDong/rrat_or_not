...
def calibrate_snr(bayes_fit, sefd):
    samples = bayes_fit.samples
    importance_weights = bayes_fit.importance_weights()
    multiplicative_factor = np.mean(sefd.calibrated_sefd_tstruct_rcvr)/np.sqrt(400e6 * 327.68e-6 * 2) #units of Jy
...
def fluxcal_fit(bayes_fit_npz, calibrator_name, ra, dec, transit_time):
    ...
    ax1.plot(sefd.freqs, sefd.calibrated_sefd_tstruct_rcvr, label='calibrated_sefd_tstruct_rcvr')
...
if __name__=='__main__':
    ...
    parser.add_argument('-transit_time', type=float, help='Transit time of the source in MJD')

    args = parser.parse_args()
    fluxcal_fit(args.bayes_fit_npz, args.calibrator_name, args.ra, args.dec, args.transit_time)
