...
def grab_spectra_manual(
    ...
    print(
        "filename:", gf, "downsample:", downsamp, "SNR:", SNR, "amp:", amp, "std", std
    )
    return SNR, amp, std
...
class inject_obj:
    ...
    def calculate_snr_single(self, mask=True):
        ts = self.toas - 5
        te = self.toas + 5
        snr, amp, std = grab_spectra_manual(
            gf=self.filfile,
            ts=ts,
            te=te,
            mask_fn=self.mask,
    ...
    def match_inj(
        ...
        for f, m in zip(self.filfiles, self.mask_fn):
            ...
            self.sorted_inject.append(
                inject_obj(snr, cur_toa, cur_dm, self.downsamp, f, m)
            )
