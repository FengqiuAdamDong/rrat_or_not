...
class det_stats:
    ...
    def create_burst(self):
        ...
        for f, m, t, d in zip(self.filfiles, self.mask_fn, self.toas, self.dms):
            t = det_obj(snr = 1, toas=t, dm=d, downsamp=self.downsamp, filfile=f, mask=m)
            temp.append(t)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-ds", type=int, help="The downsample when getting det_snr", required=True)
    ...
    init_obj = {"filfiles": fil1, "dms": dm1, "toas": toa1, "mask_fn": maskfiles, "downsamp":downsamp}
